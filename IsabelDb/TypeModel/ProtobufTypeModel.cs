...
namespace IsabelDb.TypeModel
{
		...
		: ITypeModel
	{
		...
		/// <summary>
		/// A model of all types as they were when they were last written to the database.
		/// </summary>
		private readonly TypeModel _persistedTypeModel;

		/// <summary>
		/// A model of all types as they currently are.
		/// </summary>
		...
		public ProtobufTypeModel(TypeRegistry typeRegistry)
		{
			_typeRegistry = typeRegistry ?? throw new ArgumentNullException(nameof(typeRegistry));

			_persistedTypeModel = new TypeModel();
			_currentTypeModel = new TypeModel();
		                                       ...
		                                       IEnumerable<Type> supportedTypes)
		{
			...
			var typeModel = new ProtobufTypeModel(typeRegistry);
			typeModel.Read(connection);
			typeModel.AddTypes(allTypes);
			//typeModel.ThrowIfBreakingChangesDetected();
			var compiledTypeModel = typeModel.Compile();
			typeModel.Write(connection);
			return compiledTypeModel;
		}

		private void ThrowIfBreakingChangesDetected()
		{
			_currentTypeModel.ThrowIfIncompatibleTo(_persistedTypeModel);
		}
		...
		private void AddTypes(IEnumerable<Type> supportedTypes)
		{
			...
			{
				_currentTypeModel.AddType(type);
				Add(type);
			}
		}
		...
		private void Read(SQLiteConnection connection)
		{
			...
			{
				AddTypeToTypeModel(tuple.Item2, tuple.Item1, tuple.Item3);
			}
		}
		...
		private void AddTypeToTypeModel(string typeName, int id, int? baseId)
		{
			try
			{
				...
				if (type != null)
				{
					...
					_persistedTypeModel.AddType(typeName, type, baseType, null);
